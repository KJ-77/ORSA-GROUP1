...
const OilDetail = () => {
  ...
  const addToCart = () => {
    ...
    // Check if cart is in old format (array of IDs) or new format (array of objects)
    const isOldFormat =
      cartItems.length > 0 && typeof cartItems[0] === "number";

    if (isOldFormat) {
      // Convert old format to new format
      cartItems = cartItems.map((id: number) => ({
        id: id.toString(),
        name: `Product ${id}`, // We'll need to fetch this later
        price: 0, // We'll need to fetch this later
        quantity: 1,
        image: undefined,
      }));
    }
    ...
    const existingItemIndex = cartItems.findIndex(
      (item: CartItem) => parseInt(item.id) === product.id
    );
    ...
    if (existingItemIndex !== -1) {
      ...
      const newCartItem = {
        ...
        quantity: 1,
        image: undefined, // Will be set by cart component
      };
    ...
    if (!loading && product) {
      ...
      if (productRef.current) {
        ScrollTrigger.create({
          ...
          onEnter: () => {
            const imageGallery =
              productRef.current?.querySelector(".image-gallery");
            const productInfo =
              productRef.current?.querySelector(".product-info");
        ...
        ScrollTrigger.create({
          ...
          onEnter: () => {
            const detailsContent =
              detailsRef.current?.querySelector(".details-content");
        ...
        ScrollTrigger.create({
          ...
          onEnter: () => {
            const nutritionTable =
              nutritionRef.current?.querySelector(".nutrition-table");
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-emerald-900 to-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-8 text-2xl text-white font-bold">
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-emerald-900 to-slate-900 flex items-center justify-center">
        <div className="text-center">
          <div className="backdrop-blur-md bg-red-50/20 border-2 border-red-200/30 rounded-3xl p-16 max-w-lg mx-auto shadow-2xl">
            ...
            <p className="text-red-200 mb-8 text-xl font-semibold">
        ...
        <div className="max-w-7xl mx-auto px-8 relative z-10">
          <nav className="breadcrumb backdrop-blur-md bg-white/10 rounded-2xl p-6 border border-white/20">
            <div className="flex items-center space-x-4 text-white">
              <Link
                to="/"
                className="hover:text-emerald-400 transition-colors duration-300 font-semibold"
              >
                Home
              </Link>
              ...
              <div className="grid grid-cols-4 gap-4">
                {displayImages.slice(0, 4).map((image, index) => (
                  ...
                  >
                    ...
                    />
                    <div
                      className={`absolute inset-0 bg-emerald-500/20 transition-opacity duration-300 ${
                        selectedImageIndex === index
                          ? "opacity-100"
                          : "opacity-0"
                      }`}
        ...
        <div className="max-w-7xl mx-auto px-8 relative z-10">
          ...
          <div className="details-content backdrop-blur-md bg-white/10 rounded-3xl p-12 border border-white/20 text-white">
            <p className="text-2xl leading-relaxed mb-8">
              Our Garlic Infused Olive Oil combines our premium extra virgin
              olive oil with the rich, aromatic flavor of fresh garlic. This
              infusion creates a versatile cooking oil that adds depth and
              complexity to any dish without the need to chop or mince garlic.
            </p>
            <p className="text-2xl leading-relaxed">
              We use a delicate infusion process that preserves the integrity of
              both the olive oil and the garlic flavor, resulting in a
              harmonious blend that elevates your culinary creations. Perfect
              for sautéing vegetables, drizzling over pasta, or using as a bread
              dipping oil, our Garlic Infused Olive Oil is a must-have for
              garlic lovers and home chefs alike.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {[
              ...
              <div
                key={index}
                className="related-card backdrop-blur-md bg-white/10 rounded-3xl overflow-hidden border border-white/20 text-white transform-gpu"
              >
